...
GO
PRINT N'Create table [xdb_collection].[UnlockContactIdentifiersIndex_Staging]...';
...
GO
IF NOT EXISTS (SELECT *  FROM sys.indexes  WHERE name LIKE '%IX_UnlockContactIdentifiersIndex_Staging%' 
...
SELECT @qry = 
	(SELECT  'DROP INDEX ' + ix.name + ' ON [xdb_collection].[ContactIdentifiers_Staging];'
		FROM sys.indexes AS [ix]
		WHERE
			object_id = OBJECT_ID('[xdb_collection].[ContactIdentifiers_Staging]')  
	FOR XML PATH(''));
